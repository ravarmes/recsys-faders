...
from tensorflow.keras.layers import Input, Embedding, Flatten, Dot, Dense, Concatenate, Lambda
...
class RecSysNCF:
    ...
    def fit_model(self, epochs=20, batch_size=64):
        ...
        user_ids_train, user_ids_test, item_ids_train, item_ids_test, ratings_train, ratings_test = train_test_split(user_ids, item_ids, ratings_values, test_size=0.2, random_state=42)
        ...
        output = Lambda(lambda x: x * 4 + 1)(predictions)  # Escalar para 1-5
