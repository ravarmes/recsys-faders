...
class RecSys():
    ...
    def read_dataset(self, n_users, n_items, top_users, top_items, data_dir):
        # Handle both "Data/" and "data/" cases
        if "data/" in data_dir:
            parts = data_dir.split("data/")
        elif "Data/" in data_dir:
            parts = data_dir.split("Data/")
        else:
            # If neither found, assume the last part is the dataset name
            parts = ["", data_dir.split("/")[-1]]
        
        path = parts[0]  # Contém "data/" ou "Data/"
    ...
    def  compute_X_est(self, X, algorithm='RecSysALS', data_dir="Data/MovieLens-Small"):
        if(algorithm == 'RecSysALS'):
            ...
            # Default to RecSysALS if algorithm not recognized
            rank = 1
            lambda_ = 1
            RS = RecSysALS.als_RecSysALS(rank,lambda_)
            X_est, error = RS.fit_model(X)
