...
for dataset in datasets:
    ...
    for alpha in alphas:
        ...
        for algorithm in algorithms:
            print(f"\n\n{'='*80}")
            print(f"Processing: Dataset={dataset}, Algorithm={algorithm}, Alpha={alpha}")
            print(f"{'='*80}")
            ...
            try:
                ...
                for h in hs:
                    ...
                    try:
                        ...
                        print(f"Error processing h={h} for {algorithm}: {str(e)}")
                        print(f"Traceback: {traceback.format_exc()}")
                        continue
                        
            except Exception as e:
                print(f"Error processing algorithm {algorithm} for dataset {dataset}: {str(e)}")
                print(f"Traceback: {traceback.format_exc()}")
                continue
...
print('\nThe results of the experiments were successfully saved in "_results.csv".')
print(f'Total experiments completed: {len(experiment_results)}')