...
fig.suptitle('Boxplot of the Variance of Item Losses')
...
axs[0].set_ylabel('Individual losses by item ($\u2113$)')
...
axs[1].set_xticklabels(['Before', 'After'])
axs[1].set_xlabel('State')
axs[1].set_ylabel('Individual losses by item ($\u2113$)')
...
for i, ax in enumerate(axs):
    ...
    legend_labels = [f'$R_{{indv}}$ (before): {variancia_movielens_alfa08_Xest:.2f}' if i == 0 else f'$R_{{indv}}$ (before): {variancia_goodbooks_alfa08_Xest:.2f}',
                     f'$R_{{indv}}$ (after):: {variancia_movielens_alfa08_Xpi:.2f}' if i == 0 else f'$R_{{indv}}$ (after): {variancia_goodbooks_alfa08_Xpi:.2f}']
